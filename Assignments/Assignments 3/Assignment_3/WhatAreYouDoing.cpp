 ...
 */
Set<string> allEmphasesofHelp(const Vector<string>& remaining,
                              int cnt,
                              Vector<string>& soFar){
    Set<string> ans;
    if (cnt == remaining.size()) {
        string word;
        for (string elem:remaining) word += elem;
        ans += word;
        return ans;
    } else if (soFar.size() > 0 && isalpha(remaining[cnt][0])) {
        string tem = soFar[cnt];
        soFar[cnt] = toUpperCase(soFar[cnt]);
        ans += allEmphasesofHelp(remaining,cnt+1,soFar);
        soFar[cnt] = tem;
        ans += allEmphasesofHelp(remaining,cnt+1,soFar);
        return ans;
    } else {
...
Set<string> allEmphasesOf(const string& sentence) {
    Vector<string> rem = tokenize(sentence);
    //for (string elem:rem) elem = toLowerCase(elem);
    for (int i = 0; i<rem.size();i++) rem[i] = toLowerCase(rem[i]);
    return allEmphasesofHelp(rem,0,rem);
}
...
STUDENT_TEST("This is my test cases.") {
    Set<string> expected = {
    ...
    EXPECT_EQUAL(allEmphasesOf("i love you."), expected);
}
