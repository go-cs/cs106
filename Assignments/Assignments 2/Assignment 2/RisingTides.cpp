...
//    (void) height;
//    return {};
    //初始化ans网格和terrain网格一样的规模，并且初值全部为false
    ...
    while (!q.isEmpty()) {
        GridLocation pos = q.dequeue();
        //生成一个列表来存储与当前位置距离为1的元素
        Vector<GridLocation> ps = {
            {pos.row-1, pos.col},{pos.row, pos.col+1},
            {pos.row+1, pos.col},{pos.row, pos.col-1}
        };
        for (GridLocation p:ps) {
            //还要做一个判断ps中的所有p都在有效范围内
            if (terrain.inBounds(p) && terrain.get(p) <= height && !ans.get(p)) {
                ans.set(p, true);
                q.enqueue(p);
            }
