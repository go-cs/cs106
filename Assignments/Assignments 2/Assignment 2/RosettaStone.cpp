...
//    return {};
    ...
    if (kGramLength <= 0) error("The parameter should be positive.");
    else if (str.length() < kGramLength) return {};
    for (int i = 0; i <= str.length() - kGramLength; i++){
        string str_key = str.substr(i, kGramLength);
        if (result.containsKey(str_key)) result[str_key]++ ;
        else result.put(str_key, 1);
    }
    return result;
}
...
//    return {};
    ...
    double value = 0.0;
    if (input.isEmpty()) error("Div zero!");
    else {
        for (string elem : input) {
            value += pow(input[elem], 2);
        }
        if (value == 0.0) error("Div zero");
        for (string elem:input) {
            ans[elem] = input[elem] / sqrt(value);
        }
    }
    return ans;
}
...
//    return {};
    ...
    if (numToKeep == 0) return {};
    else if (numToKeep < 0) error("The parameter should be positive.");
    else if (numToKeep > source.size()) ans = source;
    else {
        ...
        int num = pq.size();
        while (cnt < num) {
            if (cnt < num - numToKeep) pq.dequeue();
            else {
                string elem = pq.dequeue();
                ans.put(elem, source[elem]);
            }
            cnt++ ;
        }
    }
...
//    return "";
    string name = "";
    double max_cos = 0;
    if (corpora.isEmpty()) error("There is no corpora to refer to.");
    else {
        for (Corpus elem : corpora) {
            if (cosineSimilarityOf(textProfile, elem.profile) > max_cos) {
                max_cos =  cosineSimilarityOf(textProfile, elem.profile);
                name = elem.name;
            }
        }
    }
...
//student tests
STUDENT_TEST("This is the test of kGramsIn") {
    Map<string, double> test1 ={
        {"A", 1},
        {"B", 2},
        {"C", 3}
    };
    EXPECT_EQUAL(kGramsIn("ABBCCC", 1), test1);
}

STUDENT_TEST("This is the test of normalize.") {
    Map<string, double> scores = {
        {"A", 3},
        {"B", 4},
    };
